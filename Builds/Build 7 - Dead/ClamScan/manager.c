...
extern int scanmem(struct cl_engine *trie, const struct optstruct *opt, const struct cl_limits *limits, int options);
#endif
...
int scanmanager(const struct optstruct *opt)
{
    ...
    /* setup callback */
    if(opt_check(opt, "show-progress")) {
        engine->callback = scancallback;
    ...
    /* scan only memory */
    if(opt_check(opt, "memory") && (opt->filename == NULL || strlen(opt->filename) == 0))
        ret = scanmem(engine, opt, &limits, options);
    else
#endif
    /* check filetype */
